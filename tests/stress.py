...
def main():
    ...
    check(S, 'noite força dungeon perigosa (CD 15)', dcs[4:] == [15, 15], f'{dcs}')
    appmod._rate_store.clear()
    ...
    check(S, 'mestre concede caçada extra', (r.get_json() or {}).get('limit') == 7)
    r = p1.get('/api/hunts/status')
    check(S, 'status de caçadas coerente', (r.get_json() or {}).get('limit') == 7)
